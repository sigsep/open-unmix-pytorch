...
This repository contains the PyTorch implementation of __open-unmix__, a deep neural network reference implementation for music source separation, applicable for researchers, audio engineer and artists.

__open-unmix__ comes with weights, pre-trained on the [MUSDB18](https://sigsep.github.io/datasets/musdb.html) dataset.

* Design choices for _open-unmix_ favored simplicity over performance, to promote clearness of the code. The rationale is to have the model serve as a __baseline__ with performance still meeting current state-of-the-art. The results are comparable to those of `UHL1`/`UHL2`, which obtained the best performance over all systems trained on MUSDB18 in the [SiSEC 2018 Evaluation campaign](https://sisec18.unmix.app)
...
directly using `(nb_frames, nb_samples, nb_channels, nb_bins)`, where `nb_frames` and `nb_bins` are the time and frequency-dimensions of a Short-Time-Fourier-Transform.

### Normalization

At the input and output, we normalize to match the gain of the target spectrogram. This is done by using global standardization parameters (mean and standard deviation for every frequency bin across all frames) that are trainable. Furthermore, we apply batch normalization in multiple stages of the model to make the training more robust to gain variation.
...
In the following example, we have three folders with an unmatched number of vocal sounds, music sounds and noise sounds. To perform vocals separation based on music+noise interferer, you would just need to run:
...
Since PyTorch currently lacks an invertible STFT, the synthesis is performed in numpy. For inference, we rely on an implementation of a multichannel Wiener filter, that is a very popular way of filtering multichannel audio for several applications, notably speech enhancement and source separation. The `norbert` module assumes to have some way of estimating power-spectrograms for all the audio sources (non-negative) composing a mixture. It then builds the filter that is appropriate for extracting those signals from a mixture, by optimally exploiting multichannel information (like in stereo signals). This is done in an iterative procedure called Expectation Maximization, where filtering and re-estimation of the parameters are iterated.
...
`--modelname` can be used to load different pretrained models, we currently support the following:

| Model Name             | Description                                                        |
|------------------------|--------------------------------------------------------------------|
| `Unmix16kBLSTMStereo`  | Bidirectional model, Stereo, 16 kHz bandwidth (__default__)        |
| `Unmix16kLSTMMono`     | Unidirectional model, Mono, 16 kHz bandwidth (__fast alternative__) |
|------------------------|--------------------------------------------------------------------|

### Load a saved model

```bash
python test.py --modeldir /path/to/model/root/directory input_file.wav
```
