...
__open-unmix__ comes with pre-trained weights, trained on the [MUSDB18]() dataset.
...
At the input and output, we normalize to match the gain of the target spectrogram. This is done by using global standardization parameters that are trainable. Furthermore, we apply batch normalization in multiple stages of the model to make the training more robust to gain variation. 
